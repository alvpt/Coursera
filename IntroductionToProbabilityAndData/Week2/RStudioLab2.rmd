...
```{r sfo-feb-flights}
assfo_feb_flights <-  dim(sfo_feb_flights)[1]
assfo_feb_flights
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(IQR = quantile(arr_delay, 0.25) - quantile(arr_delay, 0.75))
...
nycflights %>%
  summarise(dep_dep_ot_arr_rate = sum(dep_type == "delayed" & arr_type == "on time") / n())
